...
from ui_components import (
    ...
    render_suggestions_section, feature_showcase_banner, step_header,
    back_to_hub_button, stat_card, section_divider, badge
)
...
st.set_page_config(
    page_title="Student Hub - Academic & Career AI",
    page_icon="🎓",
    layout="wide"
)
...
class ResumeApp:
    def __init__(self):
        ...
        self.pages = {
            ...
            "📌 JOB TRACKER": self.render_tracker,
        }
    ...
    def apply_global_styles(self):
        """All styles are loaded from style/style.css. No inline CSS needed."""
        pass
    ...
    def render_analyzer(self):
        ...
        page_header(
            ...
            "A guided 3-step evaluation to benchmark your resume against real tech roles and maximize your interview shortlists."
        )

        # Create tabs for Normal Analyzer and AI Analyzer
        analyzer_tabs = st.tabs(["Standard Rule-Based Analyzer", "🤖 Deep AI Analyzer"])

        with analyzer_tabs[0]:
            # ── STEP 1: JOB ROLE SELECTION ──
            step_header(1, 3, "Choose Your Target Role", "Select the industry and specific job title you are preparing your resume for.")

            col_cat, col_role = st.columns(2)
            with col_cat:
                categories = list(self.job_roles.keys())
                selected_category = st.selectbox("Industry / Domain", categories, key="standard_category")
            ...
            <div class='role-info-box'>
                <h3 style="margin-top:0;">{selected_role}</h3>
                <p>{role_info['description']}</p>
                <div style="margin-top: 0.5rem;"><strong style="color: var(--accent);">Key Required Skills:</strong></div>
                <div style="margin-top: 0.35rem;">
                    {''.join([f'<span class=\"skill-pill\">{s}</span>' for s in role_info['required_skills']])}
                </div>
            </div>
            """, unsafe_allow_html=True)
            ...
            step_header(2, 3, "Upload Your Resume", "Upload your current resume in PDF or Word format to benchmark against this role.")

            uploaded_file = st.file_uploader(
                "Choose your resume file",
                type=['pdf', 'docx'],
                key="standard_file",
                help="We extract your skills, experience, and format safely on your machine."
            )

            if not uploaded_file:
                st.info("👆 Upload your resume file above to proceed to Step 3: Analysis.")

            if uploaded_file:
                # ── STEP 3: ACTION BUTTON & RESULTS ──
                st.markdown("<br>", unsafe_allow_html=True)
                step_header(3, 3, "Generate Analysis & Action Plan", "Click below to compute your ATS score, detected skills, and missing keywords.")

                col_btn, _ = st.columns([1.2, 2])
                with col_btn:
                    analyze_standard = st.button("🔍 Run Step 3: Analyze My Resume",
                                        type="primary",
    ...
    "</ul></div>", unsafe_allow_html=True)
        ...
        with analyzer_tabs[1]:
            ...
            if use_custom_job_desc:
                ...
                st.markdown("""
                <div class='report-card' style='border-left:3px solid var(--success)'>
    ...
    size=8, line=dict(
        width=2, color='white'))
                        ...
                        if ai_stats["score_distribution"]:
                            st.markdown("""
                            <h3 style='text-align: center; margin-bottom: 20px; background: var(--accent); padding: 15px; border-radius: 10px; color: white; box-shadow: 0 4px 10px rgba(0,0,0,0.2);'>
    ...
    def render_tracker(self):
        """Render the Job Application Tracker & Kanban Dashboard"""
        render_job_tracker()
    ...
    def render_home(self):
        ...
        hero_section(
            "🎓 Student AI Hub",
            "Your all-in-one platform for academic learning and career success. Select one of the 3 primary tools below to start your step-by-step journey."
        )

        st.markdown("""
        <div class="section-divider">
            <span class="divider-label">⭐ CHOOSE YOUR PRIMARY WORKSPACE</span>
        </div>
        """, unsafe_allow_html=True)

        # ─── FEATURE 1: DOCUMENT SUMMARIZER ───
        feature_showcase_banner(
            icon="fas fa-file-alt",
            badge_label="📚 ACADEMIC STUDY TOOL",
            title="1. AI Document & Notes Summarizer",
            what_it_does="Turns dense PDF textbooks, research articles, lecture slides, and notes into structured summaries, key bullet takeaways, and flashcard review points.",
            how_it_helps="Cuts study and reading time by up to 80% and helps you master complex topics before exams with zero stress.",
            key_tags=["PDF & Word Support", "Key Takeaways", "Flashcard Notes", "Instant Export"],
            cta_label="🚀 Open Document Summarizer",
            target_page="document_summarizer",
            key="home_cta_summarizer"
        )

        # ─── FEATURE 2: RESUME ANALYZER ───
        feature_showcase_banner(
            icon="fas fa-search",
            badge_label="💼 CAREER PREPARATION",
            title="2. AI Resume Analyzer & ATS Checker",
            what_it_does="Scans your resume against 50+ tech and industry roles to calculate your ATS compatibility score and pinpoint missing skills.",
            how_it_helps="Provides an actionable step-by-step checklist to optimize your resume so you get shortlisted for tech jobs and internships.",
            key_tags=["ATS Compatibility", "Skill Gap Detection", "Role Benchmarking", "Free Courses"],
            cta_label="🔍 Open Resume Analyzer",
            target_page="resume_analyzer",
            key="home_cta_analyzer"
        )

        # ─── FEATURE 3: JOB APPLICATION TRACKER ───
        feature_showcase_banner(
            icon="fas fa-columns",
            badge_label="💼 APPLICATION PIPELINE",
            title="3. Job Application Tracker & Kanban",
            what_it_does="Organizes your entire internship and job search across visual pipeline stages: Wishlist, Applied, Interviewing, and Offers.",
            how_it_helps="Keeps interview dates, recruiter contacts, and notes in one place so you never lose track of an opportunity or follow-up deadline.",
            key_tags=["Visual Kanban Board", "Interview Prep Notes", "Deadline Alerts", "CSV Backup"],
            cta_label="💼 Open Application Tracker",
            target_page="job_tracker",
            key="home_cta_tracker"
        )
        ...
        st.markdown("""
        <div class="section-divider">
            <span class="divider-label">🛠️ COMPANION TOOLS</span>
        </div>
        """, unsafe_allow_html=True)
        ...
        with col_comp1:
            ...
            <div class="feature-card" style="text-align: center; padding: 1.25rem;">
                <div class="feature-icon" style="margin: 0 auto 0.5rem auto;"><i class="fas fa-file-signature"></i></div>
                <h4 style="margin: 0 0 0.3rem 0; color: var(--text-primary);">Resume Builder</h4>
                <p style="font-size: 0.8rem; color: var(--text-secondary); margin-bottom: 0.8rem;">Step-by-step forms to build an ATS-friendly resume from scratch.</p>
            </div>
            """, unsafe_allow_html=True)
            if st.button("Launch Builder", key="home_sub_builder", use_container_width=True):
        ...
        with col_comp2:
            st.markdown("""
            <div class="feature-card" style="text-align: center; padding: 1.25rem;">
                <div class="feature-icon" style="margin: 0 auto 0.5rem auto;"><i class="fas fa-briefcase"></i></div>
                <h4 style="margin: 0 0 0.3rem 0; color: var(--text-primary);">Job Search Portal</h4>
                <p style="font-size: 0.8rem; color: var(--text-secondary); margin-bottom: 0.8rem;">Live searches across LinkedIn, Indeed, Naukri, and top companies.</p>
            </div>
            """, unsafe_allow_html=True)
            if st.button("Search Jobs", key="home_sub_jobs", use_container_width=True):
        ...
        with col_comp3:
            st.markdown("""
            <div class="feature-card" style="text-align: center; padding: 1.25rem;">
                <div class="feature-icon" style="margin: 0 auto 0.5rem auto;"><i class="fas fa-chart-line"></i></div>
                <h4 style="margin: 0 0 0.3rem 0; color: var(--text-primary);">Career Dashboard</h4>
                <p style="font-size: 0.8rem; color: var(--text-secondary); margin-bottom: 0.8rem;">View overall readiness metrics, submission history, and analytics.</p>
            </div>
            """, unsafe_allow_html=True)
            if st.button("Open Dashboard", key="home_sub_dash", use_container_width=True):
    ...
    def main(self):
        ...
        with st.sidebar:
            ...
            st.markdown("""
                <div class="sidebar-logo">
                    ...
                    <div>
                        <div class="sidebar-logo-text">Student AI Hub</div>
                        <div class="sidebar-logo-sub">Academic & Career Suite</div>
                    </div>
                </div>
            """, unsafe_allow_html=True)
            ...
            if st.button("🏠  Hub Home", key="nav_home", use_container_width=True):
                st.session_state.page = "home"
                st.rerun()
            ...
            # CAREER SUITE
            st.markdown('<div class="sidebar-section-label">💼 CAREER & PLACEMENT</div>', unsafe_allow_html=True)
            if st.button("📌  Job Tracker (Kanban)", key="nav_tracker", use_container_width=True):
                st.session_state.page = "job_tracker"
                st.rerun()
            if st.button("🔍  Resume Analyzer", key="nav_analyzer", use_container_width=True):
                st.session_state.page = "resume_analyzer"
                st.rerun()
            if st.button("📝  Resume Builder", key="nav_builder", use_container_width=True):
                st.session_state.page = "resume_builder"
                st.rerun()
            if st.button("📊  Career Dashboard", key="nav_dashboard", use_container_width=True):
                st.session_state.page = "dashboard"
                st.rerun()
            if st.button("🎯  Job Search", key="nav_jobs", use_container_width=True):
                st.session_state.page = "job_search"
                st.rerun()
        ...
        page_render_map = {
            ...
            "resume_builder":      self.render_builder,
            "dashboard":           self.render_dashboard,
            "job_search":          self.render_job_search,
        }

        # Also support legacy cleaned names
        legacy_map = {
            name.lower().replace(" ", "_")
                ...
                .replace("📊", "").replace("🎯", "").replace("💬", "")
                .replace("ℹ️", "").strip(): fn
            for name, fn in self.pages.items()
        }

        current_page = st.session_state.get('page', 'home')

        if current_page in page_render_map:
            page_render_map[current_page]()
        elif current_page in legacy_map:
            legacy_map[current_page]()
        else:
            self.render_home()
